...
save_csv = True
mac = False
output_dir = r'C:\Users\Seb\Desktop\P-A Scripts\Prediction-Accomodation-Exp\TrialFiles\Main4-16-26'
...
def generate_trials(fmap, training_reps, participant_id):
    ...
    while mean_diff < 0.5:
        ...
        for i in range(training_reps):
            ...
            shuffled["food_score"] = shuffled.apply(
    ...
    training_df["rep_image_file"] = training_df["food_score"].astype(str) + "_food.png"
...
for df in all_data:
    ...
    mean_high_val = df_high["food_score"].mean()
    mean_low_val = df_low["food_score"].mean()
...
for df in all_data:
    ...
    rel_diff_rows.append({
        "participant_id": participant,
        "dimension": dim,
        "mean_high": df_high["food_score"].mean(),
        "mean_low": df_low["food_score"].mean(),
        "high_minus_low": df_high["food_score"].mean()
                         - df_low["food_score"].mean()
    })
