...
# --- Parameters ---
# ============================
mean_high = 7.5   # Means of distributions to draw from
mean_low = 3.5
std_dev = 1.75   # Spread of each distribution
participants = 40
save_csv = True
mac = False
#output_dir = r'/Users/sm6511/Desktop/Prediction-Accomodation-Exp/TrialFiles/Main2-7'
output_dir = r'C:\Users\Seb\Desktop\P-A Scripts\Prediction-Accomodation-Exp\TrialFiles\Main3-05-26'
...
for f, c, b in itertools.product(feet, colors, brightness):
    stimuli.append({
        "feet": f,
        "color": c,
        "brightness": b,
        "image_file": f"{c}_{f}_{b}.png"
    })
...
def sample_additive(stim_row, fmap):
    ...
    stim_row: row-like object with keys ["wing", "color"]
    fmap: feature mapping for this participant
    """
...
def parse_image_name(img_name):
    ...
    # expecting: color, brightness, feetVariant  e.g., Y_C_D
    c = parts[0]
    f = parts[1]
    b = parts[2]

    return {"feet": f, "color": c, "brightness": b}
...
while True:
    ...
    all_data = []
    irrelevant_dir_counts['feet'] = Counter()
    ...
    for participant_id in range(1, participants + 1):
        ...
        fixed_images = [
            "Resources/G_C_D.png", "Resources/G_C_L.png", "Resources/G_F_D.png", "Resources/G_F_L.png",
            "Resources/Y_C_D.png", "Resources/Y_C_L.png", "Resources/Y_F_D.png", "Resources/Y_F_L.png"
        ]
    ...
    balanced = False
    for feet in ["C", "F"]:
        high_key = f"{feet}_high"
        low_key  = f"{feet}_low"
        print(feet_counts[high_key])
        print(feet_counts[low_key])
        if feet_counts[high_key] == feet_counts[low_key]:
...
print("Proportion relevant:", 
      (total - irrelevant_counts["feet"]) / total)
